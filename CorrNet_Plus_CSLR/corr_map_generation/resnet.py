...
class Get_Correlation(nn.Module):
    def __init__(self, channels, neighbors=3, agg_mode='weighted'):
        ...
        self.clusters = 1
        # Learnable neighbor weights initialised with a distance-decay prior so that
        # closer frames start with higher weight than farther ones.
        # Layout of the L = 2*neighbors vector (mirrors UnfoldTemporalWindows / upfold concat):
        #   indices 0 … neighbors-1  → LEFT  neighbors (index 0 = farthest left)
        #   indices neighbors … L-1  → RIGHT neighbors (index neighbors = closest right)
        #
        # For neighbors=3 the distances from the current frame are:
        #   left : [3, 2, 1] (index 0→dist 3, index 2→dist 1)
        #   right: [1, 2, 3] (index 3→dist 1, index 5→dist 3)
        # We use a Gaussian with σ = neighbors/2 so the decay is smooth and adjustable.
        distances = torch.cat([
            torch.arange(self.neighbors, 0, -1, dtype=torch.float),   # left:  [N, N-1, …, 1]
            torch.arange(1, self.neighbors + 1, dtype=torch.float),    # right: [1, 2, …, N]
        ])  # shape (2*neighbors,)
        sigma = max(self.neighbors / 2.0, 0.5)
        decay_weights = torch.exp(-0.5 * (distances / sigma) ** 2)
        decay_weights = decay_weights / decay_weights.sum()  # normalise to sum=1
        self.weights2 = nn.Parameter(decay_weights, requires_grad=True)
    ...
    def forward(self, x, return_affinity=False):
        ...
        def clustering(query, key, key_spatial):
            ...
            # --- distance-decay bias -------------------------------------------------
            # Re-use weights2 (Gaussian-initialised learnable vector of shape L) as a
            # multiplicative prior: closer neighbors get a higher scale factor.
            # Softmax keeps values positive and summing to 1; multiply by L so the mean
            # scale stays at 1 (avoids globally shrinking all affinities).
            dist_scale = F.softmax(self.weights2, dim=0) * L   # (L,)
            # broadcast: (L,) -> (1,1,1,L) to match aff shape (N,T,P,L)
            aff = aff * dist_scale.view(1, 1, 1, L)
            # ------------------------------------------------------------------------
